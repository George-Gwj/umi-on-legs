...
def play():
    ...
    xml_path_ = "/home/george/code/umi-on-legs/mani-centric-wbc/resources/robots/aliengo_description/urdf/aliengo.xml"
    model = mujoco.MjModel.from_xml_path(xml_path_)
    model.opt.timestep = 0.001

    data = mujoco.MjData(model)

    data.qpos[7:] = offsets[:12]
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            config["env"]["tasks"]["locomotion"]["lin_vel_range"] = [[0.1,0.2],[0.0,0.0],[0.0,0.0]]
            ...
            if args.num_steps == -1:
                with torch.inference_mode():
                    ...
                    while True:
                        ...
                        # last_time = data.time
                        # obs_ = obs.cpu().clone().numpy()
                        # # print("actions",actions)
                        # # obs = env.step(actions)[0]
                        # action = actions_.cpu().numpy()
                        # action = np.clip(action[0,:], -max_action_value, max_action_value)


                        # obs = torch.from_numpy(obs_).to('cuda:0')

                        # if(counts <1):
                        #     target_q = offsets
                                
                        # else:
                        #     target_q = offsets + action * action_scale
                        #     # data.qpos[7:] = target_q
                        #     # obs_[0,42:78] = np.zeros(36)
                        #     # tasks
                        #     all_task_obs = []
                        #     for k, task in env.tasks.items():
                        #         task_obs = observe(task ,data)
...
def step(
    ...
    for decimation_step in range(decimation_count):
        ...
        normalized_action = offsets[:12]
        # normalized_action[12:18] = np.zeros(6)

        torques = kps[:12] * (normalized_action - data.qpos[7:]) - kps[:12] * data.qvel[6:]
    ...
    obs_[0,0:12] = data.qpos[7:] - offsets[:12]
    obs_[0,12:18] = np.zeros(6)
    obs_[0,18:30] = data.qvel[6:]
    obs_[0,30:36] = np.zeros(6)
    obs_[0,36:39] = local_root_gravity(data)
    obs_[0,39:42] = data.sensor('angular-velocity').data
    # obs_[0,42:78] = task_obs_tensor[0,:]
    # obs_[0,78:96] = action.cpu().numpy()
    obs_[0,42:49] = [ 1.0000,  0.2748,  0.0205,  0.3000, -0.0501,  0.0179, -0.9986]
...
if __name__ == "__main__":
    policy_path = "/home/george/code/umi-on-legs/mani-centric-wbc/wandb/fixedArm6d/files/model_1000.pt" 
